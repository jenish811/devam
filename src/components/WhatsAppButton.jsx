...
export default function WhatsAppButton({ productName }) {
  return (
    <div className="group fixed bottom-5 right-5 z-50 sm:bottom-6 sm:right-6">
      <span className="pointer-events-none absolute -left-28 top-1/2 hidden -translate-y-1/2 rounded-md bg-obsidian px-3 py-2 text-xs font-semibold text-white opacity-0 shadow-luxury transition group-hover:opacity-100 sm:block">
        Chat with us
      </span>
      <motion.a
        href={getWhatsAppLink({ name: productName || "Wallantq wall decor" })}
        target="_blank"
        rel="noreferrer"
        className="group relative inline-flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-luxury transition hover:brightness-95 focus:outline-none focus:ring-2 focus:ring-[#25D366] focus:ring-offset-2 dark:shadow-luxury-dark sm:h-16 sm:w-16"
